...
- Add toggle button on each transaction
- Update RLS policies:

```sql
  -- Modify the view policy to hide hidden transactions from partner
  CREATE POLICY "Users can view household transactions with privacy"
  ...
  USING (
    ...
    AND (
      user_id = auth.uid() -- Can see own transactions (hidden or not)
      OR is_hidden = false  -- Can only see partner's non-hidden transactions
    )
  );
```
